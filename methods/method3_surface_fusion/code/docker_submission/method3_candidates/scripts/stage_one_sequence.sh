...
for required in K.txt pose.txt endoscope2/L endoscope2/depthL; do
...
TEST_ROOT="$(mktemp -d "$TMP_PARENT/method3-nvs-test.XXXXXX")"
...
printf '%s\n' "method3-nvs-test-v1" > "$TEST_ROOT/.method3-nvs-test-marker"
...
echo "Staged Endoscope-2 left RGB/depth and calibration only."
printf 'TEST_ROOT=%q\n' "$TEST_ROOT"
printf 'INPUT_ROOT=%q\n' "$TEST_ROOT/input"
