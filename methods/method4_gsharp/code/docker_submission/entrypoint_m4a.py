#!/usr/bin/env python3
"""Challenge entrypoint for the frozen Method 4A G-SHARP-iMED variant.

Inference enumerates only direct input children and whitelisted source paths.
It never discovers or opens Endoscope1 RGB, depth, or masks.
"""
...
def _run(command: list[str]) -> None:
    print("[M4-A RUN]", " ".join(command), flush=True)
...
def _audit_environment() -> None:
    ...
    print("[M4-A ENV] " + json.dumps(audit, sort_keys=True), flush=True)
...
def _run_sequence(sequence: Path, output_root: Path, temporary_root: Path) -> None:
    ...
    checkpoint = train_root / sequence_name / "checkpoints" / "m4a_final.pt"
    started = time.perf_counter()
    try:
        _run(
            [
                sys.executable,
                str(METHOD_ROOT / "train_imed.py"),
                "--data-root",
                str(sequence.parent),
                "--sequence",
                sequence_name,
        ...
        # The directory was created by this process beneath the fixed Method 4
        # temporary root. Never clean an unresolved or user-supplied path.
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Frozen iMED NVS Method 4A submission")
...
def main() -> int:
    ...
    print(f"[M4-A] discovered {len(sequences)} legal sequence(s)", flush=True)
