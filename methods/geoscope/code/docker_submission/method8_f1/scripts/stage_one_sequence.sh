...
for required in K.txt pose.txt endoscope2/L endoscope2/depthL endoscope2/toolL; do
    if [[ ! -e "$SOURCE_SEQUENCE/$required" ]]; then
        echo "Missing required legal source input: $SOURCE_SEQUENCE/$required" >&2
        exit 1
    fi
done

TEST_ROOT="$(mktemp -d "$TMP_PARENT/method8-f1-nvs-test.XXXXXX")"
...
printf '%s\n' "method8-f1-nvs-test-v1" > "$TEST_ROOT/.method8-f1-nvs-test-marker"
...
echo "Staged only legal source-side inputs; no Endoscope-1 tree was copied."
printf 'TEST_ROOT=%q\n' "$TEST_ROOT"
printf 'INPUT_ROOT=%q\n' "$TEST_ROOT/input"
